...
>
	<div class="flex items-center justify-between gap-4">
		...
		<h1 class="text-lg font-semibold md:text-2xl">Публични събития</h1>
	</div>
	...
	{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.CreateEvents)}
		<Button
			...
			href="/control-centre-v1/public-events/create"
		>
...
{#if !(data.events && data.events.length > 0)}
	...
	>
		<div class="flex flex-col items-center gap-4 text-center">
			...
			{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.CreateEvents)}
				<Button class="flex items-center gap-2" href="/control-centre-v1/public-events/create">
					<CirclePlus class="h-5 w-5 max-md:h-4 max-md:w-4" /> Ново събитие
				</Button>
			{/if}
		</div>
