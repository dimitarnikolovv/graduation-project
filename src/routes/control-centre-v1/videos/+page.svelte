...
>
	<div class="flex items-center justify-between gap-4">
		...
		<h1 class="text-lg font-semibold md:text-2xl">Видеа</h1>
	</div>

	{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.UploadVideos)}
		<Button size="sm" class="flex items-center gap-2 " href="/control-centre-v1/videos/upload">
			<Upload class="h-5 w-5 max-md:h-4 max-md:w-4" /> Качи видео
		</Button>
	{/if}
</div>
...
{#if !(data.videos && data.videos.length > 0)}
	...
	>
		<div class="flex flex-col items-center gap-4 text-center">
			...
			{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.UploadVideos)}
				<Button class="flex items-center gap-2 " href="/control-centre-v1/videos/upload">
					<Upload class="h-5 w-5 max-md:h-4 max-md:w-4" /> Качи видео
				</Button>
			{/if}
		</div>
...
{:else}
	<div class="grid gap-3 max-lg:gap-y-8 lg:grid-cols-2 xl:grid-cols-3">
		{#each data.videos as video}
			<Video {video} userPermissions={data.userPermissions} />
		{/each}
	</div>
