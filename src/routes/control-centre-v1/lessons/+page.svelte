...
>
	<div class="flex items-center justify-between gap-4">
		...
		<h1 class="text-lg font-semibold md:text-2xl">Уроци</h1>
	</div>

	{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.CreateLessons)}
		<Button size="sm" class="flex items-center gap-2 " href="/control-centre-v1/lessons/create">
			<CirclePlus class="h-5 w-5 max-md:h-4 max-md:w-4" /> Нов урок
		</Button>
	{/if}
</div>
...
{#if data.lessons.length === 0}
	...
	>
		<div class="flex flex-col items-center gap-4 text-center">
			...
			{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.CreateLessons)}
				<Button class="flex items-center gap-2 " href="/control-centre-v1/lessons/create">
					<CirclePlus class="h-5 w-5 max-md:h-4 max-md:w-4" /> Нов урок
				</Button>
			{/if}
		</div>
...
{:else}
	<div class="grid grid-cols-1 gap-4 max-lg:gap-y-6 lg:grid-cols-2 xl:grid-cols-3">
		{#each data.lessons as lesson}
			<LessonCard {lesson} userPermissions={data.userPermissions} />
		{/each}
	</div>
