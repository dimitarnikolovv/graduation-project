<script lang="ts">
	...
	import { getTests } from './data.remote';
	import TestCard from './TestCard.svelte';

	type Props = {
		selectedTestId: string | null | undefined;
	...
	const { tests, totalItems, totalPages } = $derived(
		await getTests({
			page,
			limit,
			searchParams
	...
	{:else}
		<div class="grid gap-3 max-lg:gap-y-8 lg:grid-cols-2 xl:grid-cols-3">
			{#each tests as test (test.id)}
				<TestCard {test} bind:selectedTestId {disabled} />
			{/each}
		</div>
