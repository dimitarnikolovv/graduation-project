<script lang="ts">
	...
	import { deletePaidEventRemote } from './actions.remote';
	...
	async function deleteCurrentEvent() {
		...
		try {
			const result = await deletePaidEventRemote({
				id: event.id
			});
		...
		<div class="flex w-full justify-between">
			<div class="min-w-0 flex-1">
				<Card.Title class="text-lg leading-tight">
					<a
						href="/control-centre-v1/paid-events/{event.id}/edit"
	...
	<Card.Footer>
		<div class="flex w-full items-center flex-wrap justify-between gap-2">
			<Button
				size="sm"
				variant="outline"
				href="/control-centre-v1/paid-events/{event.id}/entries"
