import { command, getRequestEvent, query } from '$app/server';
import { checkIfUserAndRole } from '$lib/server/auth';
import { db } from '$lib/server/db';
import { deleteUploadedFile } from '$lib/server/server-utils/files';
import { RolesEnum, UserPermissionsEnum } from '$lib/types/enums';
import { checkIfUserHasPermission } from '$lib/utils/access-control';
import { error } from '@sveltejs/kit';
import { and, eq, isNotNull, isNull } from 'drizzle-orm';
import z from 'zod';
import { asString, generateCsv } from 'export-to-csv';
import { paidEvents } from '$lib/server/db/schema/paidEvents';
import { paidEventEntries } from '$lib/server/db/schema/paidEventEntries';

const deletePaidEventSchema = z.object({
	id: z.uuid()
});

export const deletePaidEventRemote = command(deletePaidEventSchema, async ({ id }) => {
	const { locals } = getRequestEvent();

	if (!checkIfUserAndRole(locals, [RolesEnum.admin])) {
		return { success: false, code: 401, message: 'Отказан достъп.' };
	}

	if (!checkIfUserHasPermission(locals.userPermissions, UserPermissionsEnum.DeleteEvents)) {
		return {
			success: false,
			code: 403,
			message: 'Нямате нужните права за изтриване на публични събития.'
		};
	}

	try {
		await db.transaction(async (tx) => {
			const [deletedEvent] = await tx.delete(paidEvents).where(eq(paidEvents.id, id)).returning();

			if (deletedEvent.posterFileId) {
				await deleteUploadedFile(deletedEvent.posterFileId, tx);
			}
		});

		return { success: true, code: 200, message: 'Събитието беше изтрито успешно.' };
	} catch (err) {
		console.log('Error deleting public event:', err);
		return {
			success: false,
			code: 500,
			message: 'Възникна грешка при изтриването на събитието.'
		};
	}
});

const deletePosterSchema = z.object({
	eventId: z.string().min(1, 'Event ID е задължително поле.')
});

export const deletePaidEventPosterRemote = command(deletePosterSchema, async ({ eventId }) => {
	const { locals } = getRequestEvent();

	if (!checkIfUserAndRole(locals, [RolesEnum.admin])) {
		return { success: false, code: 401, message: 'Отказан достъп.' };
	}

	if (!checkIfUserHasPermission(locals.userPermissions, UserPermissionsEnum.EditEvents)) {
		return { success: false, code: 403, message: 'Отказан достъп.' };
	}

	try {
		await db.transaction(async (tx) => {
			const post = await tx.query.paidEvents.findFirst({
				where: eq(paidEvents.id, eventId),
				columns: {
					id: true,
					posterFileId: true
				}
			});

			if (!post) {
				throw new Error('Събитието не беше намерено.', { cause: 404 });
			}

			if (!post.posterFileId) {
				throw new Error('Събитието няма прикачен постер.', { cause: 400 });
			}

			// Delete the file from S3
			await deleteUploadedFile(post.posterFileId);

			// Update the paid event to remove the poster reference
			await tx.update(paidEvents).set({ posterFileId: null }).where(eq(paidEvents.id, post.id));
		});

		return { success: true, code: 200, message: 'Постерът беше изтрит успешно.' };
	} catch (err) {
		console.error(err);
		if (err instanceof Error && err.cause === 404) {
			return { success: false, code: 404, message: 'Събитието не беше намерено.' };
		}
		if (err instanceof Error && err.cause === 400) {
			return { success: false, code: 400, message: 'Събитието няма прикачен постер.' };
		}
		return { success: false, code: 500, message: 'Възникна грешка при изтриването на постера.' };
	}
});

const deletePaidEventEntrySchema = z.object({
	id: z.uuid()
});

export const deletePaidEventEntryRemote = command(deletePaidEventEntrySchema, async ({ id }) => {
	const { locals } = getRequestEvent();

	if (!checkIfUserAndRole(locals, [RolesEnum.admin])) {
		return { success: false, code: 401, message: 'Отказан достъп.' };
	}

	if (!checkIfUserHasPermission(locals.userPermissions, UserPermissionsEnum.EditEvents)) {
		return { success: false, code: 403, message: 'Отказан достъп.' };
	}

	try {
		await db.delete(paidEventEntries).where(eq(paidEventEntries.id, id));
		return { success: true, code: 200, message: 'Участникът беше изтрит успешно.' };
	} catch (err) {
		console.error('Error deleting entry:', err);
		return { success: false, code: 500, message: 'Възникна грешка при изтриването на участника.' };
	}
});

const downloadPaidEventEntriesSchema = z.object({
	eventId: z.uuid(),
	format: z.enum(['csv', 'txt']).default('csv'),
	target: z.enum(['paid', 'unpaid', 'all']).default('all')
});

export const downloadPaidEventEntriesRemote = query(
	downloadPaidEventEntriesSchema,
	async ({ eventId, format, target }) => {
		const { locals } = getRequestEvent();

		if (!checkIfUserAndRole(locals, [RolesEnum.admin])) {
			error(403, 'Отказан достъп.');
		}

		if (!checkIfUserHasPermission(locals.userPermissions, UserPermissionsEnum.ViewEvents)) {
			error(403, 'Нямате нужните права за преглед на публични събития.');
		}

		// Get event details
		const event = await db.query.paidEvents.findFirst({
			where: eq(paidEvents.id, eventId)
		});

		if (!event) {
			error(404, 'Събитието не беше намерено.');
		}

		function buildEntryFilter() {
			const query = [eq(paidEventEntries.eventId, eventId)];

			if (target === 'paid') {
				query.push(isNotNull(paidEventEntries.paidAt));
			} else if (target === 'unpaid') {
				query.push(isNull(paidEventEntries.paidAt));
			}

			return and(...query);
		}

		const entryFilter = buildEntryFilter();

		// Get all entries (no pagination for download)
		const entries = await db.query.paidEventEntries.findMany({
			where: entryFilter,
			columns: {
				createdAt: true,
				firstName: true,
				lastName: true,
				atendeeEmail: true
			},
			orderBy: (paidEventEntries, { asc }) => [asc(paidEventEntries.createdAt)]
		});

		let content: string = '';
		let contentType: string;
		let filename: string;

		if (format === 'csv') {
			const csv = generateCsv({
				useKeysAsHeaders: true
			})(
				entries.map((entry) => ({
					email: entry.atendeeEmail,
					name: entry.firstName + ' ' + entry.lastName,
					createdAt: entry.createdAt.toISOString()
				}))
			);

			content = asString(csv);

			contentType = 'text/csv';
			filename = `${event.name.replace(/[ ]/gi, '_')}_emails.csv`;
		} else {
			// TXT format - one email per line
			content = entries.map((entry) => entry.atendeeEmail).join('\n');
			contentType = 'text/plain';
			filename = `${event.name.replace(/[ ]/gi, '_')}_emails.txt`;
		}

		return {
			content: content,
			contentType: contentType,
			filename: filename
		};
	}
);

const markPaidEventEntryAsPaidSchema = z.object({
	entryId: z.uuid()
});

export const markPaidEventEntryAsPaidRemote = command(
	markPaidEventEntryAsPaidSchema,
	async ({ entryId }) => {
		const { locals } = getRequestEvent();

		if (!checkIfUserAndRole(locals, [RolesEnum.admin])) {
			return { success: false, code: 401, message: 'Отказан достъп.' };
		}

		if (!checkIfUserHasPermission(locals.userPermissions, UserPermissionsEnum.EditEvents)) {
			return { success: false, code: 403, message: 'Отказан достъп.' };
		}

		try {
			const [updatedEntry] = await db
				.update(paidEventEntries)
				.set({
					paidAt: new Date()
				})
				.where(eq(paidEventEntries.id, entryId))
				.returning();

			return {
				success: true,
				code: 200,
				message: 'Участникът беше отбелязан като платил успешно.',
				entry: updatedEntry
			};
		} catch (err) {
			console.error('Error marking entry as paid:', err);
			return {
				success: false,
				code: 500,
				message: 'Възникна грешка при отбелязването на участника като платил.'
			};
		}
	}
);
