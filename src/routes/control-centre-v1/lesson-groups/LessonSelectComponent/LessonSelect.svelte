<script lang="ts">
	...
	import { getLessons } from './data.remote';
	...
	const { lessons, totalItems, totalPages } = $derived(
		await getLessons({
			page,
			limit,
			searchParams
...
<div class="mt-4 grid gap-4">
	...
	{:else}
		<div class="grid grid-cols-1 gap-3 max-lg:gap-y-8 lg:grid-cols-2 xl:grid-cols-3">
			{#each lessons as lesson (lesson.id)}
				<LessonCard {lesson} bind:selectedLessonIds {disabled} />
			{/each}
		</div>
