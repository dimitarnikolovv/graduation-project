<script lang="ts">
	...
	import { downloadPaidTestEntriesRemote } from '$lib/remote-functions/tests.remote';
	...
	async function downloadEmails(format: 'csv' | 'txt') {
		...
		downloadPaidTestEntriesRemote({ testId: data.test.id, format, target: 'unpaid' })
			.then((response) => {
...
{#if !(data.entries && data.entries.length > 0)}
	...
	>
		<div class="flex flex-col items-center gap-4 text-center">
			<h3 class="text-2xl font-bold tracking-tight">Няма незаплатили участници</h3>
			<p class="text-muted-foreground">Няма незаплатили участници за това събитие.</p>
		</div>
	</div>
{:else}
	<EntriesBlock
