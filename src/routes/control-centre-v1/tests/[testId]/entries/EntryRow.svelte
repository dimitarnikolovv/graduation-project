<script lang="ts">
	...
	import {
		deletePaidTestEntryRemote,
		markPaidTestEntryAsPaidRemote
	} from '$lib/remote-functions/tests.remote';
	import type { FetchPaidTestEntriesResult } from '$lib/server/db-querying/paidTestEntries';

	type Props = {
		entry: FetchPaidTestEntriesResult['results'][number];
		index: number;
		page: number;
		limit: number;
	...
	async function deleteCurrentEntry() {
		...
		try {
			await deletePaidTestEntryRemote({
				id: entry.id
			});

			toast.success('Участникът беше изтрит успешно.');
			await invalidateAll();
		} catch (error) {
			console.error(error);
	...
	async function markAsPaid() {
		...
		try {
			await markPaidTestEntryAsPaidRemote({
				entryId: entry.id
			});

			toast.success('Успешно маркиран като платено.');
			await invalidateAll();
		} catch (error) {
			console.error(error);
...
<Table.Row>
	...
	<Table.Cell class="font-mono">
		<a href="mailto:{entry.user.email}">{entry.user.email}</a></Table.Cell
	>
	...
	<ResponsiveDialog bind:open={openDelete} closeButtonText="Отказ" {disabled}>
		...
		{#snippet content()}
			<div>
				...
				<br />
				<span class="font-mono font-bold">{entry.user.email}</span>
			</div>
		{/snippet}
