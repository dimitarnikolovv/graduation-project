...
>
	...
	<div class="flex items-center gap-2">
		{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.EditTests)}
			<AddEntryDialog data={data.addEntryForm} testId={data.test.id} />
		{/if}
	</div>
</div>
...
<Card.Root class="mb-4">
	<Card.Header>
		<Card.Title>{data.test.title}</Card.Title>
	...
	<Card.Content>
		<div class="flex flex-wrap items-center gap-4">
			{#if data.test.opensAt}
				<Badge variant="secondary" class="text-sm">
					<Calendar class="mr-2 h-4 w-4" />
					{dfEventDate.format(new Date(data.test.opensAt))}

					{#if data.test.closesAt}
						- {dfEventDate.format(new Date(data.test.closesAt))}
					{/if}
				</Badge>
			{/if}
...
<PageNav.Root>
	<PageNav.Item
		...
		href="/control-centre-v1/tests/{data.test.id}/entries"
	>
	...
	<PageNav.Item
		class="max-xl:flex-row-reverse"
		href="/control-centre-v1/tests/{data.test.id}/entries/unpaid"
	>
