<script lang="ts">
	/**
	 * Pending Attempts Page
	 *
	 * Lists all attempts that are awaiting grading from teachers/admins.
	 */
...
{#if data.attempts && data.attempts.length > 0}
	<div class="space-y-3">
		{#each data.attempts as attempt (attempt.id)}
			<AttemptRow {attempt} showGradeButton={true} />
		{/each}
	</div>
...
{:else}
	...
	>
		<ClipboardList class="mb-4 h-12 w-12 text-gray-400" />
		<h3 class="mb-2 text-lg font-semibold text-gray-900 dark:text-gray-100">
			Няма опити за оценяване
		</h3>
		<p class="text-sm text-gray-500 dark:text-gray-400">
			Всички предадени тестове са оценени.
		</p>
	</div>
{/if}
