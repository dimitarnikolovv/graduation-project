...
>
	<div class="flex items-center justify-between gap-4">
		...
		<h1 class="text-lg font-semibold md:text-2xl">Тестове</h1>
	</div>
	...
	<div class="flex items-center gap-2">
		<Button
			...
			href="/control-centre-v1/tests/attempts"
		>
			<ClipboardCheck class="h-4 w-4" /> Оценяване

			{#if data.testAttemptsForGradingCount > 0}
				<Badge
					variant="outline"
					class="flex h-5 shrink-0 animate-pulse items-center justify-center rounded-full bg-yellow-50 text-yellow-700"
				>
					{formatLargeNumber(data.testAttemptsForGradingCount)}
				</Badge>
			{/if}
		</Button>

		{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.CreateTests)}
			<Button size="sm" class="flex items-center gap-2 " href="/control-centre-v1/tests/create">
				<CirclePlus class="h-5 w-5 max-md:h-4 max-md:w-4" /> Нов тест
			</Button>
		{/if}
	</div>
</div>

<Filters subjects={data.subjects} classGrades={data.classGrades} />
...
{#if !(data.tests && data.tests.length > 0)}
	...
	>
		<div class="flex flex-col items-center gap-4 text-center">
			...
			{#if checkIfUserHasPermission(data.userPermissions, UserPermissionsEnum.CreateTests)}
				<Button class="flex items-center gap-2 " href="/control-centre-v1/tests/create">
					<CirclePlus class="h-5 w-5 max-md:h-4 max-md:w-4" /> Нов тест
				</Button>
			{/if}
		</div>
...
{:else}
	<div class="grid gap-4 max-lg:gap-y-6 lg:grid-cols-2 xl:grid-cols-3">
		{#each data.tests as test (test.id)}
			<TestCard {test} userPermissions={data.userPermissions} />
		{/each}
	</div>
