...
<svelte:head>
	<title>Предстоящи безплатни събития | BRAAND</title>
	<meta
		name="description"
		content="Разгледайте предстоящите безплатни образователни събития и се запишете за участие."
	/>
</svelte:head>

<div class="mb-8">
	<h1 class="heading-font text-titles mb-4 text-4xl font-bold">Предстоящи безплатни събития</h1>
	<p class="text-muted-foreground text-lg">
		Запишете се за безплатни образователни събития, семинари и уъркшопи
	</p>
</div>
...
<PageNav.Root class="mb-8">
	<PageNav.Item
		...
		href="/events/public?filter=upcoming"
	...
	<PageNav.Item
		class="justify-center"
		href="/events/public?filter=past"
		...
		{#each data.events as event}
			<Card.Root class="flex flex-col gap-y-0 overflow-hidden pt-0 transition-all hover:shadow-lg">
				...
				<Card.Header class="mt-6">
					<Badge variant={isEventUpcoming(event.date) ? 'default' : 'secondary'} class="mb-2 w-fit">
						{isEventUpcoming(event.date) ? 'Предстоящо' : 'Приключило'}
					</Badge>
					<Card.Title class="line-clamp-2 text-lg">
						<a href="/events/public/{event.id}" class="hover:underline">{event.name}</a>
					</Card.Title>
				...
				<Card.Footer class="mt-auto">
					<Button href="/events/public/{event.id}" class="w-full" size="sm">
