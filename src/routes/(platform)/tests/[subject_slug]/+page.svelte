...
<svelte:head>
	...
	<meta
		name="description"
		content="Изберете клас за тестове по {data.subject.name}."
	/>
</svelte:head>
	...
	{#if !data.gradesForSubject || data.gradesForSubject.length === 0}
		...
		>
			<div class="flex flex-col items-center gap-4 text-center">
				...
				<h3 class="text-2xl font-bold tracking-tight">Не намерихме тестове</h3>
				<p class="text-muted-foreground max-w-md">
					За съжаление, все още няма налични тестове по {data.subject.name}. Това скоро ще се промени,
	...
	{:else}
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 lg:grid-cols-3 xl:grid-cols-4">
			{#each data.gradesForSubject as grade}
				<a
					href="/tests/{data.subject.slug}/{grade.slug}"
