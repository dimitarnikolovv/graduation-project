<script>
	...
	export let webkitdirectory = false;
	...
	export let disableClicks = false;
...
<div
	...
	class="{disableDefaultStyles
		? ''
		: 'dropzone relative isolate flex aspect-square w-full cursor-pointer items-center justify-center overflow-hidden rounded-md border border-dashed'}
  {className}"
...
>
	<input
		...
		style="display: none;"
		class={disableClicks ? 'pointer-events-none' : ''}
	/>
	<slot>
		<Upload class="text-muted-foreground absolute h-4 w-4" />
	</slot>
</div>
