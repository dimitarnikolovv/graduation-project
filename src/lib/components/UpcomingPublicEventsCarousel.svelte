<script lang="ts">
	...
	type EventWithPoster = PublicEvent & {
		...
		<div class="mb-8 flex flex-wrap items-center justify-between gap-4">
			<div>
				<h2 class="heading-font text-titles mb-2 text-2xl font-bold sm:text-3xl lg:text-4xl">
					Предстоящи безплатни събития
				</h2>
				<p class="text-muted-foreground">Запишете се за безплатни образователни събития</p>
			</div>
			<Button href="/events/public" variant="outline" class="max-sm:w-full">
				Всички безплатни събития
				...
				{#each events as event}
					<Carousel.Item class="md:basis-1/2 xl:basis-1/4">
						<Card.Root class="group h-full overflow-hidden pt-0 transition-all hover:shadow-md">
							...
							{/if}
							<Card.Header>
								<Badge variant="default" class="mb-2 w-fit">Предстоящо</Badge>
							...
							<Card.Footer>
								<Button href="/events/public/{event.id}" class="w-full">Запиши се</Button>
							</Card.Footer>
						</Card.Root>
					</Carousel.Item>
				{/each}
